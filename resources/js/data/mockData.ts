...
export const getActivities = () => {
    ...
    let apps: any[] = [];
    try {
        jobs = JSON.parse(localStorage.getItem('mock_jobs_custom') || '[]');
        apps = JSON.parse(localStorage.getItem('mock_applications_custom') || '[]');
    } catch(e) {}
    ...
    jobs.forEach((job: any) => {
        ...
        activities.push({
            id: `act_${idCounter++}`,
            action: 'Job Posted',
            details: `New position: ${job.title} (${job.employmentType})`,
    ...
    apps.forEach((app: any) => {
        ...
        if (app.status === 'Hired') {
            ...
            const rejectedDateStr = dateObj.toISOString().split('T')[0];
            ...
            const shortStr = dateObj.toISOString().split('T')[0];
            
            activities.push({
                id: `act_${idCounter++}`,
                action: 'Candidate Shortlisted',
                details: `${app.applicantName} shortlisted for ${app.jobTitle}`,
